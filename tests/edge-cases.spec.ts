...
test.describe('Omaze UK - Edge Cases & Error Handling', () => {
  ...
  test('should handle network interruptions gracefully', async ({ page }) => {
    ...
    await page.getByRole('link', { name: 'Enter Now' }).first().click({ timeout: 10000 });
  ...
  test('should handle direct URL access to various pages', async ({ page }) => {
    ...
    const pagesToTest = [
      { url: 'https://omaze.co.uk/pages/enter-cheshire-iii', expectedText: 'YOUR CHANCE TO WIN' },
      { url: 'https://omaze.co.uk/pages/faqs', expectedText: 'Frequently Asked Questions' },
    ...
    for (const pageTest of pagesToTest) {
      ...
      await expect(page.locator(`text=${pageTest.expectedText}`)).toBeVisible({ timeout: 10000 });
    }
  });
  ...
  test('should handle subscription tier edge cases', async ({ page }) => {
    ...
    for (const detail of subscriptionDetails) {
      ...
      await expect(page.getByRole('button', { name: detail.button })).toBeVisible();
    }
