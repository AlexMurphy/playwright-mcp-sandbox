...
test.describe("Omaze UK - Accessibility & Performance Tests", () => {
  ...
  test("should have proper page titles and meta information", async ({
    page,
  }) => {
    // Check homepage title
    await expect(page).toHaveTitle(
      /Your Chance to Win a House in Cheshire.*Omaze UK/,
    );
    ...
    await expect(page).toHaveTitle(
      /Your Frequently Asked Questions, Answered.*Omaze UK/,
    );
  });
  ...
  test("should have proper image alt text and media accessibility", async ({
    page,
  }) => {
  ...
  test("should handle different viewport sizes", async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      if (viewport.width < 768) {
        ...
        await expect(
          page.getByRole("link", { name: "Past Draws" }),
        ).toBeVisible();
      }
    }
  });
  ...
  test("should have proper color contrast and visual accessibility", async ({
    page,
  }) => {
    // Take screenshot for manual contrast checking
    await page.screenshot({
      path: "tests/screenshots/homepage-contrast.png",
      fullPage: true,
    });
    ...
    await page.getByRole("link", { name: "Enter Now" }).first().click();
    await page.screenshot({
      path: "tests/screenshots/entry-page-contrast.png",
      fullPage: true,
    });
  });
});
