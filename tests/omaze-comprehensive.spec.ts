...
test.describe("Omaze UK - Comprehensive Test Suite", () => {
  ...
  test("should display main house draw information on homepage", async ({
    page,
  }) => {
  ...
  test("should navigate to entry page and show all entry methods", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Postal No purchase necessary" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Single Purchase" }),
    ).toBeVisible();
    await expect(
  ...
  test("should switch between entry method tabs correctly", async ({
    page,
  }) => {
    await page.getByRole("link", { name: "Enter Now" }).first().click();
  ...
  test("should navigate to FAQs and verify content organization", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h2:has-text("The Cheshire House")'),
    ).toBeVisible();
    await expect(page.locator('h2:has-text("Anthony Nolan")')).toBeVisible();
  });

  test("should verify header navigation links work correctly", async ({
    page,
  }) => {
  ...
  test("should verify charity information is prominently displayed", async ({
    page,
  }) => {
  ...
  test("should verify footer links and social media integration", async ({
    page,
  }) => {
    // Verify footer links are functional
    const footer = page.locator("footer");
    await expect(
      footer.getByRole("link", { name: "About Omaze" }),
    ).toBeVisible();
    ...
    await expect(
      footer.getByRole("link", { name: "Official Rules" }),
    ).toBeVisible();
  ...
  test("should verify Trustpilot integration and compliance", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('img[alt*="Chartered Institute of Fundraising"]'),
    ).toBeVisible();
    await expect(
      page.locator('img[alt*="Fundraising Regulator"]'),
    ).toBeVisible();
  });

  test("should verify draw timing and Early Bird prize information", async ({
    page,
  }) => {
  ...
  test("should verify house gallery and property information", async ({
    page,
  }) => {
    // Verify house gallery and tour functionality
    await expect(
      page.getByRole("tab", { name: "Tour", selected: true }),
    ).toBeVisible();
    await expect(page.getByRole("tab", { name: "Gallery" })).toBeVisible();
    await expect(
      page.getByRole("tab", { name: "Floorplans (1/2)" }),
    ).toBeVisible();
    await expect(
      page.getByRole("tab", { name: "Floorplans (2/2)" }),
    ).toBeVisible();
  ...
  test("should verify account and authentication features", async ({
    page,
  }) => {
  ...
  test("should handle cookie consent mechanism", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Accept Essential Only" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Customise Cookies" }),
    ).toBeVisible();
  ...
  test("should verify charity and fundraising information", async ({
    page,
  }) => {
    await page.getByRole("link", { name: "Enter Now" }).first().click();
    ...
    await expect(
      page.locator("text=Over one third of sales goes to charity and VAT"),
    ).toBeVisible();
