...
test.describe("Omaze UK - User Journey Tests", () => {
  ...
  test("complete entry flow - single purchase journey", async ({ page }) => {
    ...
    await expect(
      page.locator(
        "text=Every entry you make into the Cheshire House Draw helps Anthony Nolan",
      ),
    ).toBeVisible();

    // Check buy now buttons are present
    await expect(
      page.getByRole("button", { name: "Buy Now" }).first(),
    ).toBeVisible();
  });
  ...
  test("complete entry flow - subscription journey", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Enter Now" })).toHaveCount(
      3,
    );
  });
  ...
  test("postal entry journey", async ({ page }) => {
    ...
    await expect(
      page.locator(
        "text=On a blank sheet of paper, or postcard, write or type your full, legal name",
      ),
    ).toBeVisible();
    await expect(
      page.locator(
        "text=Postal entries are treated in exactly the same way as paid entries",
      ),
    ).toBeVisible();
  });
  ...
  test("early bird prize information flow", async ({ page }) => {
    ...
    await page
      .getByRole("link", { name: "McLaren Artura Spider | Cheshire" })
      .click();
  ...
  test("charity information deep dive", async ({ page }) => {
    ...
    await expect(
      page.locator(
        "text=Omaze guarantees to make a minimum charitable donation of £1,000,000",
      ),
    ).toBeVisible();
  });
  ...
  test("footer links comprehensive check", async ({ page }) => {
    ...
    await expect(
      footer.locator('[href*="facebook.com/OmazeUK"]'),
    ).toBeVisible();
    await expect(
      footer.locator('[href*="instagram.com/omazeuk"]'),
    ).toBeVisible();
